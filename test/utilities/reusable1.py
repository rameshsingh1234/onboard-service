...
def read_csv_entity():
    ...
    #         requester_info = payload_dict.get('requester', {})
    #         aatest_entity = requester_info.get('id')
    #         aatest_kid = requester_info.get('kid')
    #         if aatest_entity is not None:
    #             test_data_entity_ids.append(aatest_entity)
    #             test_kid_id.append((k))
    #             print(f'AAtestEntity: {aatest_entity}')
    #
    # if set(cr_entity_ids) == set(test_data_entity_ids):
    #     new_entity_id = str(uuid.uuid4())
    ...
    #     return same_entity_id
        for row in csv_reader:
            payload = row['payload']
            payload_dict = json.loads(payload)
            requester_info = payload_dict.get('entityinfo', {})
            aatest_entity = requester_info.get('certificate',{})
            aatest_kid = aatest_entity.get('kid')

            if aatest_kid is not None:
                test_data_entity_ids.append(aatest_kid)
                test_kid_id.append((k))
                print(f'AAtestEntity: {aatest_kid}')
